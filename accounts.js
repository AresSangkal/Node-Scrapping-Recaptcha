...
function startTask(taskNum) {
	...
	request({
		...
		if (response.statusCode === 403) {
			...
			let s = $('input[name="r"]').attr('value');
			console.log("@@@@@@@@@@@@@@@@@@@@@@")
			console.log(sitekey, id, s);
			console.log("##############################")
...
function getCaptcha(sitekey, url, id, s, callback) {
	console.log(captchaKey,sitekey,url);
	request({
		...
		solveCaptcha(captchaId, function(body) {
			...
			request({
				...
				if (!error) {
					console.log("EEEEEEEEEEEERRRRRRRRRRRRRROOOOOOOOOOOOOOOOORRRRRRRRRRRRRR")
					callback();
				}
			});
		});
